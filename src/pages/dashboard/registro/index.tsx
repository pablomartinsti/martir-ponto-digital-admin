...
function RegistroHoras() {
  ...
  return (
    <Container>
      <FiltroWrapper>
        <SelectFuncionario value={employeeId} onChange={(e) => setEmployeeId(e.target.value)}>
      ...
      {!loading && registros.length === 0 && employeeId && (
        <p className="text">Nenhum registro encontrado para o período selecionado.</p>
      )}

      {registros.length > 0 && (
        <>
          <Button style={{ width: '200px', margin: '1rem 0' }} onClick={() => window.print()}>
            Imprimir Registro
          </Button>
          ...
          <div>
            ...
            <CardResumo className="card-resumo">
              <h2>
                Funcionário: <strong>{funcionarios.find((f) => f._id === employeeId)?.name}</strong>
              </h2>
              ...
              <tbody>
                {registros.map((registro) => (
                  <tr key={registro.date}>
                    <td>{registro.clockIn ? formatarData(registro.clockIn) : formatarData(registro.date)}</td>
                    <td>{registro.clockIn ? formatarHorario(registro.clockIn) : '-'}</td>
                    <td>{registro.lunchStart ? formatarHorario(registro.lunchStart) : '-'}</td>
                    <td>{registro.lunchEnd ? formatarHorario(registro.lunchEnd) : '-'}</td>
                    <td>{registro.clockOut ? formatarHorario(registro.clockOut) : '-'}</td>
                    <td>{registro.workedHours}</td>
                    <td className={registro.balance?.startsWith('-') ? 'saldo-negativo' : 'saldo-positivo'}>
                      {registro.balance}
                    </td>
                    ...
                    <td className="col-status">
                      {' '}
                      {podeJustificar.includes(registro.status.toLowerCase()) && !isHoje(registro.date) && (
                        <Button
                          style={{
                            height: 20,
                            marginTop: '4px',
                        ...
                        >
                          {registro.justified ? 'Editar justificativa' : 'Justificar'}
                        </Button>
                      )}
                    </td>
                  </tr>
            ...
            <TableMobile>
              {registros.map((registro) => (
                <div className="card" key={registro.date}>
                  <p>
                    <strong>Data:</strong>{' '}
                    {registro.clockIn ? formatarData(registro.clockIn) : formatarData(registro.date)}
                  </p>
                  <p>
                    <strong>Entrada:</strong> {registro.clockIn ? formatarHorario(registro.clockIn) : '-'}
                  </p>
                  <p>
                    <strong>Início do Almoço:</strong>{' '}
                    {registro.lunchStart ? formatarHorario(registro.lunchStart) : '-'}
                  </p>
                  <p>
                    <strong>Fim do Almoço:</strong> {registro.lunchEnd ? formatarHorario(registro.lunchEnd) : '-'}
                  </p>
                  <p>
                    <strong>Saída:</strong> {registro.clockOut ? formatarHorario(registro.clockOut) : '-'}
                  </p>
                  <p>
                    <strong>Horas:</strong> {registro.workedHours}
                  </p>
                  <p className={registro.balance?.startsWith('-') ? 'saldo-negativo' : 'saldo-positivo'}>
